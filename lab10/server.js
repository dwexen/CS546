...
app.use("/api", function(req, response, next) {
    ...
    logData.addLog(req.requestId,req.path,req.method,req.cookies, time).then((theLog) => {
      response.json(theLog);
      next()
    });

});

app.get("/cookies/addCookie", function(req, res, next) {
    if(typeof req.query.key === 'string' && typeof req.query.value === 'string')
    {
      res.cookie(req.query.key, req.query.value);
      res.status(200).json("This succeeded");
    }
    else
    {
      res.status(500).json("Enter both key and value as stringsfor both key and value");
    }
    next();
});
...
app.get("/api/movies/best", function(request, response) {
    movieData.getPopularMovies().then(function(popularMovies) {
        var me = response.json(popularMovies);
        console.log(me);
    });
});
